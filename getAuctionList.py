        ...
        while True:
            ...
            for item in itemList:
                ...
                try:
                    ...
                    for tr in estateListTbody.find_elements(By.TAG_NAME, "tr"):
                        ...
                        if tds[1].text == "토지":
                            ...
                            for line in detailOfList.split("\n"):
                                ...
                                # 토지
                                # 면적
                                if (line.startswith("대") or line.startswith("임야") or line.startswith("구거") or line.startswith("잡종지") or line.startswith("묘지") or line.startswith("도로") or line.startswith("공장용지") or line.startswith("답") or line.startswith("하천") or line.startswith("전")) and line.endswith("㎡"):
                                ...
                                if "." in line and "분의" in line and "매각지분" not in line:
                                    # numerator = float(
                                    #     re.findall(r'([0-9]*)\s*분의\s*([0-9]*)', line)[0][0])
                                    # denominator = float(
                                    #     re.findall(r'([0-9]*)\s*분의\s*([0-9]*)', line)[0][1])

                                    numerator = float(re.findall(
                                        r'(\d+(?:\.\d+)?)\s*분의\s*(\d+(?:\.\d+)?)', line)[0][0])
                                    denominator = float(
                                        re.findall(r'(\d+(?:\.\d+)?)\s*분의\s*(\d+(?:\.\d+)?)', line)[0][1])
                    ...
                    print("Complated insert: ", caseNumber)
...
def job():
    itemType = ["아파트", "오피스텔", "근린생활시설", "다세대주택", "단독주택", "대지",
                "임야", "전", "답", "공장", "도로"]
